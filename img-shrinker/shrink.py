...
def shrinker(files, size):
    ...
    for file in files:
        proc = Popen(
            [cmd, file, "-define", "jpeg:extent={0}".format(size), "{0}_shrunk.jpg".format(file)],
            stdout=PIPE,
            stderr=PIPE,
        )
...
def main():
    ...
    parser.add_argument('size', help='Max size to shrink images to', choices=['1kb', '500kb', '1mb', '10mb', '100mb'])
